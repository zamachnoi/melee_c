...
static void update(sha256_ctx_t *ctx, const uint8_t *data, size_t len) {
    ...
    while (len) {
        ...
        if (ctx->used == 64) { transform(ctx, ctx->block); ctx->used = 0; }
    }
}

static void finish(sha256_ctx_t *ctx, uint8_t digest[32]) {
    ctx->block[ctx->used++] = 0x80;
    if (ctx->used > 56) {
        memset(ctx->block + ctx->used, 0, 64 - ctx->used);
        transform(ctx, ctx->block); ctx->used = 0;
    }
    memset(ctx->block + ctx->used, 0, 56 - ctx->used);
    for (unsigned i = 0; i < 8; i++) ctx->block[63-i] = (uint8_t)(ctx->bits >> (i*8));
...
void sha256_bytes(const void *data, size_t len, uint8_t digest[32]) {
    sha256_ctx_t ctx; init(&ctx); update(&ctx, data, len); finish(&ctx, digest);
}

int sha256_file(const char *path, uint8_t digest[32]) {
    FILE *f = fopen(path, "rb");
    if (!f) return -1;
    sha256_ctx_t ctx; init(&ctx);
    uint8_t buffer[65536];
    size_t n;
    while ((n = fread(buffer, 1, sizeof buffer, f)) != 0) update(&ctx, buffer, n);
    if (ferror(f)) { fclose(f); return -1; }
    fclose(f); finish(&ctx, digest); return 0;
}
